...
// 列表数据
const tableData = ref<PaymentApi.PaymentItem[]>([]);
...
// 表单数据
const formData = ref<
  PaymentApi.CreatePaymentParams | PaymentApi.UpdatePaymentParams
>({
  name: "",
  code: "",
  gateway: 1,
...
const formRules = {
  ...
  code: [{ required: true, message: "请输入支付代码", trigger: "blur" }],
  gateway: [{ required: true, message: "请选择支付网关", trigger: "change" }],
};
...
const loadData = async () => {
  try {
    loading.value = true;
    const params: PaymentApi.GetPaymentListParams = {
      ...searchForm.value,
...
// 打开编辑对话框
const handleEdit = (row: PaymentApi.PaymentItem) => {
  isEdit.value = true;
  dialogTitle.value = "编辑支付";
  formData.value = {
    id: row.id,
    name: row.name,
    code: row.code,
...
const handleSubmit = async () => {
  try {
    ...
    loading.value = true;
    if (isEdit.value) {
      await updatePaymentApi(formData.value as PaymentApi.UpdatePaymentParams);
    } else {
      await createPaymentApi(formData.value as PaymentApi.CreatePaymentParams);
    }
...
// 删除
const handleDelete = async (row: PaymentApi.PaymentItem) => {
  try {
    await ElMessageBox.confirm(`确定要删除支付"${row.name}"吗？`, "提示", {
    ...
    loading.value = true;
    await deletePaymentApi({ id: row.id });
  ...
  <div class="p-4">
    ...
    <ElCard class="mb-4" shadow="never">
      <div class="flex items-center gap-4 mb-4">
        ...
        <ElButton type="success" @click="handleAdd">新增支付</ElButton>
      </div>
    </ElCard>
    ...
    <ElCard shadow="never">
      ...
      >
        ...
        <ElTableColumn prop="name" label="支付名称" width="150" />

        <ElTableColumn prop="code" label="支付代码" width="120" />
    ...
    >
      <ElForm
        ...
        label-width="100px"
      >
        <ElFormItem label="支付名称" prop="name">
          <ElInput v-model="formData.name" placeholder="请输入支付名称" />
        </ElFormItem>

        <ElFormItem label="支付代码" prop="code">
          <ElInput v-model="formData.code" placeholder="请输入支付代码" />
        </ElFormItem>

        <ElFormItem label="支付网关" prop="gateway">
          <ElSelect v-model="formData.gateway" placeholder="请选择支付网关">
            <ElOption
              v-for="item in gatewayOptions"
              :key="item.value"
              :label="item.label"
        ...
        <ElFormItem label="支付域名">
          <ElInput v-model="formData.api_url" placeholder="请输入支付域名" />
        </ElFormItem>
